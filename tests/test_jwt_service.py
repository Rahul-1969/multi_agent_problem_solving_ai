...
def _reload_jwt_service():
    """Reload backend.auth.jwt_service so module-level secrets pick up the
    current environment. Must be called from inside each test method after
    the autouse set_env fixture has run and after any env mutations."""
...
class TestSecretKeyConfiguration:
    ...
    def test_runtime_error_when_jwt_secret_key_missing(self, monkeypatch):
        ...
        jwt_module = _reload_jwt_service()
    ...
    def test_imports_successfully_when_jwt_secret_key_set(self, monkeypatch):
        ...
        monkeypatch.delenv("JWT_REFRESH_SECRET_KEY", raising=False)
        jwt_module = _reload_jwt_service()
        from backend.auth.jwt_service import SECRET_KEY
        assert SECRET_KEY == "a-valid-secret-key"
...
class TestTokenCreation:
    ...
    def test_access_token_expiration(self):
        ...
        token = create_access_token("alice")
        secret = os.environ["JWT_SECRET_KEY"]
        from backend.auth.jwt_service import ALGORITHM
        payload = jwt.decode(token, secret, algorithms=[ALGORITHM])
