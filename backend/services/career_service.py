...
def get_career_advice(query: str, username: str, context: str = "") -> Optional[AIResult]:
    """
    Fetches career advice using the configured provider.
    """
    if not usage_tracker.can_use(username, "flash"):
        logger.warning(f"User {username} reached flash limits.")
        return None
        
    cache_key = f"career:{query}"
    cached_content = gemini_cache.get(cache_key, GEMINI_FLASH_MODEL)
    if cached_content:
        metrics.record_cache_hit("career")
        return AIResult(
    ...
    # Read prompt
    template = load_prompt("career_prompt.txt")
    if template:
        formatted_prompt = template.format(context=context, query=query)
    else:
        # Fallback if prompt file is missing
        formatted_prompt = f"Career query: {query}\nContext: {context}"
    
    result = provider.generate(prompt=formatted_prompt, system="You are an expert career counselor.")
